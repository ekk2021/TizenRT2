...
i2c_t	 i2cslave;
 ...
 //(Master Mode) Input Multiple Slaves I2C Addresses array here
 uint8_t slaves[] = { 0x23 };
#else
 
 //I2C (Slave mode)
 //Give an unique address identifier for each slave
...
//		 DBG_8195A("i2c data: %02x \t %02x\n",i2cdatarddst[i2clocalcnt],i2cdatarddst[i2clocalcnt+1]);
 ...
 void main_i2c_example(void)
 {
	 ...
	 int total_slaves = sizeof(slaves) / sizeof(slaves[0]);
 
	 while(1) {
		 for(int index = 0; index < total_slaves; index++) {
...
#ifdef I2C_RESTART_DEMO
			rtk_i2c_write(&i2cmaster, slaves[index], (char*) &i2cdatasrc[0], 1, 0);
			rtk_i2c_write(&i2cmaster, slaves[index], (char*) &i2cdatasrc[1], (I2C_DATA_LENGTH-1), 1);
#else
			rtk_i2c_write(&i2cmaster, slaves[index], (char*) &i2cdatasrc[0], I2C_DATA_LENGTH, 1);
#endif
			...
			rtk_i2c_write(&i2cmaster, slaves[index], (char*) &i2cdatasrc[0], 1, 0);
#endif
			rtk_i2c_read(&i2cmaster, slaves[index], (char*) &i2cdatarddst[0], I2C_DATA_LENGTH, 1);
#else
#ifdef I2C_RESTART_DEMO
			 i2c_write(&i2cmaster, slaves[index], (char*) &i2cdatasrc[0], 1, 0);
			 i2c_write(&i2cmaster, slaves[index], (char*) &i2cdatasrc[1], (I2C_DATA_LENGTH-1), 1);
#else
			 i2c_write(&i2cmaster, slaves[index], (char*) &i2cdatasrc[0], I2C_DATA_LENGTH, 1);
#endif
			 // Master read - Slave write
			 DBG_8195A("Master read>>>\n");
#ifdef I2C_RESTART_DEMO
			 i2c_write(&i2cmaster, slaves[index], (char*) &i2cdatasrc[0], 1, 0);
#endif
			 i2c_read(&i2cmaster, slaves[index], (char*) &i2cdatarddst[0], I2C_DATA_LENGTH, 1);
#endif
			 i2c_master_rx_check();
			 DelayMs(1000);
		 }
	 ...
	 //Required for Slave write repetition
	 I2C_SLAVEWRITE_PATCH = 1;
