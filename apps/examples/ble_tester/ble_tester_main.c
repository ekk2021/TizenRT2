...
#define BLE_STATE_MANAGER_RMC_HANDLE_OTA_NOTI_CCCD (0xff03)
...
static ble_server_gatt_t gatt_profile[] = {
	...
	{.type = BLE_SERVER_GATT_DESC, .uuid = {0x02,0x29}, .uuid_length = 2,
	.permission = BLE_ATTR_PERM_R_PERMIT|BLE_ATTR_PERM_W_PERMIT,
	.attr_handle = BLE_STATE_MANAGER_RMC_HANDLE_OTA_NOTI_CCCD, .cb = utc_cb_desc_b_1, .arg = "desc_4"},
};
...
static int ble_write_timing(int count, int interval, int packet_size)
{
	ble_result_e ret;
	ble_client_state_e cli_state;
	...
	RMC_LOG(RMC_CLIENT_TAG, "BLE is connected [ %s ]\n", __client_state_str(cli_state));
	RMC_LOG(RMC_CLIENT_TAG, "Send %d Bytes packet %d times.\n", packet_size, count);
	RMC_LOG(RMC_CLIENT_TAG, "Interval : %d ms\n", interval);

	ble_attr_handle attr_handle = BLE_APP_HANDLE_CHAR_RMC_KEY + 1;
	ble_data packet[1] = { 0, };
	uint8_t packet_data[256] = { 0, };
	ble_packet_s ble_packet[1];
	...
	for (i = 0; i < 10; i++) {
		ioctl(frt_fd, TCIOC_GETSTATUS, (unsigned long)(uintptr_t)&before);
		RMC_LOG(RMC_CLIENT_TAG, "test : %09ld\n", (unsigned long)before.timeleft);
	...
	for (i = 1; i <= count; i++) {
		ioctl(frt_fd, TCIOC_GETSTATUS, (unsigned long)(uintptr_t)&before);
		ble_packet->id = i;
		ble_packet->packet_size = packet_size;
		ble_packet->total = count;
		ble_packet->tiemstamp = (unsigned long)before.timeleft;
		memcpy(packet_data, ble_packet, sizeof(ble_packet_s));
		packet_data[packet_size - 1] = i % sizeof(uint8_t);
		ret = ble_client_operation_write(g_ctx, attr_handle, packet);
		RMC_LOG(RMC_CLIENT_TAG, "Timestamp : %09ld\n", ble_packet->tiemstamp);
		if (ret != BLE_MANAGER_SUCCESS) {
			send_fail++;
			RMC_LOG(RMC_CLIENT_TAG, "Fail[X] (%d / %d) [%d]\n", i, count, ret);
		} else {
		...
		ioctl(frt_fd, TCIOC_GETSTATUS, (unsigned long)(uintptr_t)&after);
		diff = (unsigned long)after.timeleft - (unsigned long)before.timeleft;
		RMC_LOG(RMC_CLIENT_TAG, "diff : %09ld\n", diff);
		if (diff < interval * 1000) {
			usleep((interval * 1000) - diff);
		}
	}
...
int ble_tester_main(int argc, char *argv[])
{
	...
	// ioctl(frt_fd, TCIOC_GETSTATUS, (unsigned long)(uintptr_t)&before);
	// ioctl(frt_fd, TCIOC_GETSTATUS, (unsigned long)(uintptr_t)&after);
	// unsigned long log_diff = (unsigned long)after.timeleft - (unsigned long)before.timeleft;
	...
	if (argc < 2) {
		...
		*/
		...
		(void)wifi_manager_init(&g_wifi_callbacks);
		usleep(100 * 1000);
		is_setup = 1;
	...
	if (argc >= 2) {
		if (strncmp(argv[1], "init", 5) == 0) {
		...
		} else if (strncmp(argv[1], "set", 6) == 0) {
			/*
			...
			*/
		...
		} else if (strncmp(argv[1], "test10", 7) == 0) {
		} else if (strncmp(argv[1], "test11", 7) == 0) {
		} else if (strncmp(argv[1], "test12", 7) == 0) {
		} else if (strncmp(argv[1], "timing", 7) == 0) {
			int packet_size = 244;
			int count = 10;
			int interval = 50;
			...
			ble_write_timing(count, interval, packet_size);
		} else {
